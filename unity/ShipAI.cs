...
namespace UnityStandardAssets.Vehicles.Aeroplane
{
    ...
    public class ShipAI : MonoBehaviour
    {
        ...
        public float ThrottleInput = 0.1f;
        ...
        private void FixedUpdate()
        {
            if (m_Target != null)
            {
                ...
                Vector3 targetPos = m_Target.position +
                                    transform.right*
                                    (Mathf.PerlinNoise(m_LateralWanderSpeed, m_RandomPerlin)*2 - 1)*
                ...
                var rollInput = -(m_AeroplaneController.RollAngle - desiredRoll)*m_RollSensitivity;

                // pass the current input to the plane (false = because AI never uses air brakes!)
                m_AeroplaneController.Move(rollInput, pitchInput, ThrottleInput, false);
            }
            else
            {
                // no target set, send zeroed input to the planeW
                m_AeroplaneController.Move(0, 0, 0, false);
            }
        }
